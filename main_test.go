...
func insert(db *sql.DB, events []*eventqueue.Event) error {
	...
	statement, err := tx.Prepare(`
		INSERT INTO pg2kafka.outbound_event_queue (external_id, table_name, statement, data, previous_data, processed)
		VALUES ($1, $2, $3, $4, $5, $6)
	`)
	if err != nil {
	...
	for _, e := range events {
		_, serr := statement.Exec(e.ExternalID, e.TableName, e.Statement, e.Data, e.PreviousData, e.Processed)
		if serr != nil {
